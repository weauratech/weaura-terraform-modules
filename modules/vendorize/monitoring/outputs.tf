...
output "iam_mode" {
  ...
  value       = local.effective_iam_mode
}

output "iam_role_arn" {
  description = "IAM role ARN for monitoring stack service accounts"
  value = var.cloud_provider == "aws" ? (
    local.effective_iam_mode == "irsa" ? try(aws_iam_role.monitoring[0].arn, null) :
    local.effective_iam_mode == "pod_identity" ? try(aws_iam_role.monitoring_pod_identity[0].arn, null) : null
  ) : null
}

output "iam_role_name" {
  description = "IAM role name for monitoring stack service accounts"
  value = var.cloud_provider == "aws" ? (
    local.effective_iam_mode == "irsa" ? try(aws_iam_role.monitoring[0].name, null) :
    local.effective_iam_mode == "pod_identity" ? try(aws_iam_role.monitoring_pod_identity[0].name, null) : null
  ) : null
}

output "pod_identity_role_arn" {
  description = "ARN of the Pod Identity IAM role (null if iam_mode is irsa)"
  value       = local.effective_iam_mode == "pod_identity" && var.cloud_provider == "aws" ? try(aws_iam_role.monitoring_pod_identity[0].arn, null) : null
}

output "pod_identity_associations" {
  description = "Map of Pod Identity associations by component"
  value = local.effective_iam_mode == "pod_identity" && var.cloud_provider == "aws" ? {
    loki      = var.loki.enabled ? try(aws_eks_pod_identity_association.loki[0].id, null) : null
    mimir     = var.mimir.enabled ? try(aws_eks_pod_identity_association.mimir[0].id, null) : null
    tempo     = var.tempo.enabled ? try(aws_eks_pod_identity_association.tempo[0].id, null) : null
    pyroscope = var.pyroscope.enabled ? try(aws_eks_pod_identity_association.pyroscope[0].id, null) : null
  } : {}
}
...
output "service_accounts" {
  description = "Kubernetes service accounts created for monitoring (AWS only)"
  value = var.cloud_provider == "aws" ? {
...
output "enterprise_features" {
  ...
  value = {
    pdb             = var.pdb_enabled
    alert_rules     = var.alert_rules_enabled
    network_policy  = var.network_policy_enabled
    tls             = var.tls_enabled
    alertmanager    = var.alertmanager_enabled
    service_monitor = var.service_monitor_auto_enable
  }
}
